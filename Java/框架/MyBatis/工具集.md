...
1. name是`user[0].department.name`，此时prop的结构是{indexedName=user[0]},children=department.name}
2. 当进入到`metaObjectForProperty(String name)`方法时，实际上就会递归进行`getValue(String name)`调用，此时新建的prop的结构是
